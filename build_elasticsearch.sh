...
function installJava() {
		...
		export ES_JAVA_HOME=/usr/lib64/jvm/java-${jver}-openjdk-${jver}
            ...
            printf "%s is not supported, Please use valid java from {Temurin17, OpenJDK17, Temurin21, OpenJDK21} only\n" "$JAVA_PROVIDED"
            exit 1
        fi
}
...
function configureAndInstall() {
	...
	make build
        ...
        mkdir -p $CURDIR/elasticsearch/libs/native/libraries/build/platform/linux-s390x/
        cp $CURDIR/zstd-$ZSTD_VERSION/lib/libzstd.so $CURDIR/elasticsearch/libs/native/libraries/build/platform/linux-s390x/
        ls $CURDIR/elasticsearch/libs/native/libraries/build/platform/linux-s390x/
        ...
        if [[ $JAVA_PROVIDED =~ 21$ ]]; then
            ...
            sudo cp $CURDIR/zstd-$ZSTD_VERSION/lib/libzstd.so /usr/share/elasticsearch/lib/platform/linux-s390x/
        fi
